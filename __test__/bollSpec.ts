...
describe('BOLL', () => {
  ...
  it('Boll', () => {
    ...
    expect(returnedResults.upper.map(formatT)).toEqual(upper)
    expect(returnedResults.lower.map(formatT)).toEqual(lower)
    expect(returnedResults.mid.map(formatT)).toEqual(mid)
    expect(returnedResults.bandWidth.map(formatT)).toEqual(bandWidth)
  })
});
